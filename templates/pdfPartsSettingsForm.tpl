                                ...
                                {foreach from=$filesInformation key=partName item=fileInfo}
                                    ...
                                    {if $ext == 'png' || $ext == 'jpg' || $ext == 'jpeg' || $ext == 'svg' || $ext == 'tiff'}
                                        <tr style="border-top: 1px solid #e5e7eb;">
                                            ...
                                            <td style="padding: 0.75rem 1rem; font-size: 0.875rem; color: #6b7280;">{if !$fileInfo.public}<a onclick="return confirm('Deleting {$fileInfo.filename}');" href="{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="generic" plugin=$pluginName verb="resetPart" partName=$fileInfo.filename template=$selectedTemplate}" style="text-decoration: none; padding: 0.6rem 1.2rem; background-color: #aa0707ff; color: #ffffff; font-weight: 600; font-size: 0.95rem; border: none; border-radius: 6px; cursor: pointer; transition: background-color 0.2s;">{translate key="plugins.generic.jatsParser.pdf.reset"}</a>{else}<div style="display:flex;gap:0.5rem;"><input type="file" name="{$partName|escape}" /><input type="hidden" name="partName[]" value="{$partName|escape}" /></div>{/if}</td>
                                        </tr>
                                    {/if}
                                {/foreach}
                            </tbody>
