                                ...
                                {foreach from=$filesInformation key=partName item=fileInfo}
                                    ...
                                    {if $ext == 'png' || $ext == 'jpg' || $ext == 'jpeg' || $ext == 'svg' || $ext == 'tiff'}
                                        <tr style="border-top: 1px solid #e5e7eb;">
                                            ...
                                            <td style="padding: 0.75rem 1rem; font-size: 0.875rem; color: #6b7280;">{if !$fileInfo.public}<a href="{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="generic" plugin=$pluginName verb="resetPart" partName=$fileInfo.filename template=$selectedTemplate}" style="color:#ef4444;">{translate key="plugins.generic.jatsParser.pdf.reset"}</a>{else}<div style="display:flex;gap:0.5rem;"><input type="file" name="{$partName|escape}" /><input type="hidden" name="partName[]" value="{$partName|escape}" /></div>{/if}</td>
                                        </tr>
                                    {/if}
                                {/foreach}
                            </tbody>
