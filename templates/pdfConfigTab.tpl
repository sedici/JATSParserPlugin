<script>
    $(function() {ldelim}
        // $('#jatsParserPartsForm').pkpHandler('$.pkp.controllers.form.AjaxFormHandler');
    {rdelim});
</script>

<form class="pkp_form" id="jatsParserPartsForm" method="post" action="{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="pdfPartsSettings" plugin=$pluginName verb="test" save=true}" enctype="multipart/form-data">
    {csrf}
                        ...
                        {foreach from=$filesInformation key=partName item=fileInfo}
                            <tr style="border-bottom: 1px solid #e5e7eb;">
                                ...
                                <td style="padding: 1rem 1.25rem; vertical-align: middle; font-size: 0.875rem; color: #6b7280;">
                                    {if !$fileInfo.public}
                                        <a href="{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="generic" plugin=$pluginName verb="resetPart" partName=$partName}" class="pkp_link pkp_link_reset">
